...
const pool = mysql.createPool({
    ...
    database: 'heroku_c200ea6bfbe06be',
    waitForConnections: true,
    connectionLimit: 10,
    queueLimit: 0,
});
...
// 로컬로 할때는 hongmovie, Heroku 를 사용할때는 gitsearch
const emailDoesExist = async (email) => {
    const QUERY = 'SELECT email from gitsearch where email = ?';
...
const storeUserInfo = async (firstname, password, email) => {
    const QUERY = 'INSERT INTO gitsearch(firstname, password, email, date) VALUES (?,?,?, current_timestamp)';
...
exports.signinWithToken = async (req, res, next) => {
    try {
        ...
        const getUserDataFromDB = async (email) => {
            const QUERY = "SELECT email,password, firstname FROM gitsearch where email =?";
            const [rows] = await promisePool.query(QUERY, email);
